    ...
    <div class="hero-wrap">
      <div class="hero-left">
        ...
        <div class="hero-actions">
          <button
            class="btn-submit-modern btn-hero"
    ...
    <div class="bento-grid">
      <div class="reab-card card-2 efficiency-gradient">
        <div class="d-flex justify-content-between align-items-start gap-3">
          <div class="min-w-0">
            <p
              class="small fw-900 opacity-90 text-uppercase mb-2"
          ...
          <div class="pill-soft"><CheckCircle :size="16" /> Live</div>
        </div>
        ...
        <div
          class="mt-3 d-flex justify-content-between x-small fw-800 opacity-90"
        >
      ...
      <div class="reab-card">
        <p
          class="small fw-900 text-muted text-uppercase mb-2"
          style="letter-spacing: 1px"
        >
          ការងារដែលនៅសល់
        </p>
      ...
      <div class="reab-card">
        ...
        <div class="calendar-grid">
          ...
          <div
            v-for="p in calendarData.padding"
            :key="'p' + p"
            class="cal-date empty"
          ></div>
          <div
            v-for="date in calendarData.days"
            :key="date"
            class="cal-date"
...
const inProgressCount = computed(
  () => tasks.value.filter((t) => !isTaskDone(t)).length
);
...
const series = computed(() => {
  ...
  const counts = days.map((day) => {
    ...
    return tasks.value.filter((t) => {
      const v = t?.createdAt ?? t?.created_at;
      if (!v) return false;
...
const chartOptions = computed(() => ({
  ...
  xaxis: {
    categories: last7Days().map(
      (d) => daysKh[d.getDay() === 0 ? 6 : d.getDay() - 1]
    ),
...
const exportTasks = () => {
  const blob = new Blob([JSON.stringify(tasks.value, null, 2)], {
...
// Calendar Logic
const now = new Date();
const currentCalendarHeader = computed(() => {
...
/* Active Teal Style (Matches Dashboard) */
.is-today {
...
.cal-date:hover:not(.is-today):not(.empty) {
