...
export const useAuthStore = defineStore("auth", () => {
  ...
  const changePassword = async (currentPassword, newPass, confirmPass) => {
    ...
    try {
      // Build payload with proper field names that match API expectations
      const payload = {
      ...
      if (err.response?.status === 400) {
        ...
        profileError.value =
          ...
          err.response?.data?.error ||
          err.message ||
          "បរាជ័យក្នុងការផ្លាស់ប្តូរពាក្យសម្ងាត់។ សូមព្យាយាមម្ដងទៀត";
      }
      profileLoading.value = false;
      return false;
    }
  ...
  const deleteAccount = async (password) => {
    ...
    try {
      ...
      console.log("Step 1: Confirming account deletion with password");

      const response = await api.post("/auth/delete-account", payload);

      console.log("Step 2: Delete account response received");
      console.log("Response status:", response.status);
      console.log("Response data:", response.data);
      ...
      if (
        ...
        console.log(
          "Step 3: Account deletion successful, clearing local storage",
        );

        // Clear all auth data in correct order
        token.value = null;
        user.value = null;
        profile.value = null;
        localStorage.removeItem("token");
        ...
        profileSuccess.value = "គណនីបានលុបដោយជោគជ័យ។";
        profileLoading.value = false;

        console.log("Step 4: Cleanup complete, ready for redirect");
        return true;
      } else {
        profileError.value = response.data?.message || "មិនអាចលុបគណនីបានទេ។";
        profileLoading.value = false;
        return false;
      }
    } catch (err) {
      ...
      if (err.response?.status === 400) {
        ...
        if (errorMsg && errorMsg.includes("password")) {
          profileError.value = "ពាក្យសម្ងាត់មិនត្រឹមត្រូវ។";
        } else {
          profileError.value = errorMsg || "សូមបញ្ចូលទិន្នន័យត្រឹមត្រូវ។";
        }
      } else if (err.response?.status === 401 || err.response?.status === 403) {
        profileError.value = "មិនមានសិទ្ធិលុបគណនី។ សូមលេងម្តងទៀត។";
      } else if (err.response?.status === 422) {
        profileError.value = "ទិន្នន័យមិនឆ្លុះឆ្លងទេ។ សូមព្យាយាមម្តងទៀត។";
      } else {
        profileError.value =
          err.response?.data?.message ||
          err.response?.data?.error ||
