    ...
    <Transition name="fade">
      <div v-if="showDeleteModal" class="modal-backdrop">
        <div class="modal-card">
          ...
          <div class="modal-body">
            <p>
              សូមបញ្ចូលពាក្យសម្ងាត់របស់អ្នក ដើម្បីបញ្ជាក់ការលុបគណនី
          ...
          <div class="modal-foot">
            ...
            <button
              class="btn-danger"
              :disabled="
                !deleteConfirmation.password || authStore.profileLoading
              "
        ...
        <div class="modal-card" @click.stop>
          ...
          <div class="modal-foot">
            ...
            <button
              ...
              :disabled="authStore.profileLoading">
              {{ authStore.profileLoading ? "កំពុងផ្ញើ..." : "ផ្លាស់ប្តូរ" }}
            </button>
          </div>
        </div>
      </div>
...
const deleteAccountConfirmed = async () => {
  ...
  if (!deleteConfirmation.value.password) {
    authStore.profileError = "សូមបញ្ចូលពាក្យសម្ងាត់។";
    return;
  }

  // Step 2: Call delete account API
  console.log("Step 1: Starting account deletion process");
  const result = await authStore.deleteAccount(
    deleteConfirmation.value.password,
  );

  if (result) {
    console.log("Step 2: Account deletion successful");
    authStore.profileSuccess = "គណនីបានលុបដោយជោគជ័យ។";
    deleteConfirmation.value.password = "";

    // Step 3: Close modal and redirect after 1.5 seconds
    setTimeout(async () => {
      console.log("Step 3: Closing modal");
      showDeleteModal.value = false;

      // Step 4: Clear auth store completely
      console.log("Step 4: Logging out user");
      authStore.logout();
    ...
    console.error("Step 2 Failed: Account deletion failed");
  }
};
